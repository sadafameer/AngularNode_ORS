...
class UserService extends BaseService {
    ...
    search(user, pageNo, pageSize, callback) {
        ...
        super.executeSQLForList(sql, { "pageNo": pageNo, "pageSize": pageSize}, new User(), function (err, list) {
            callback(err, list);
        });
    }
  ...
  register(user, callback) {
    ...
    //     super.executeSQLForObject(sql, params, new User(), function (err, result) {
    //         if (result == undefined) {
    //             var sql = " INSERT INTO st_user (CREATED_DATETIME,MODIFIED_DATETIME,FIRST_NAME,LAST_NAME,LOGIN,PASSWORD,DOB,MOBILE_NO,ROLE_ID,GENDER) " + " VALUES (NOW(),NOW(),?,?,?,?,?,?,?,?)";
    //         var params = [user.firstName, user.lastName, user.login, user.password, user.dob, user.mobileNo, user.roleId, user.gender];
    ...
    add(user, callback) {
        ...
        super.executeSQLForObject(sql, params, new User(), function (err, data) {
            if (data == undefined) {
                ...
                console.log("inside userservice add method" + result);
                callback(result, err);
            }
        });
    };
